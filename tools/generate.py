...
def get_tts_wav(
    ...
    with torch.no_grad():
        ...
        wav16k = torch.as_tensor(wav16k, device=device).half() if is_half else torch.as_tensor(wav16k, device=device)
        zero_wav_torch = zero_wav_torch.half() if is_half else zero_wav_torch
